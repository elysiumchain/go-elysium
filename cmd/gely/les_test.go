...
func (g *gelyrpc) killAndWait() {
	g.gely.Kill()
	g.gely.WaitExit()
}

func (g *gelyrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gely.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gelyrpc) addPeer(peer *gelyrpc) {
	g.gely.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gely.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gely.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gely.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gely.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gelyrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *gelyrpc) waitSynced() {
	...
	if err != nil {
		g.gely.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.gely.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gely.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gely.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gely.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGelyWithIpc(t *testing.T, name string, args ...string) *gelyrpc {
	ipcName := fmt.Sprintf("gely-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	ipcpath := ipcEndpoint(ipcName, g.gely.Datadir)
	// We can't know exactly how long gely will take to start, so we try 10
...
func initGely(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gely: %v ", args)
	g := runGely(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gelyrpc {
	datadir := initGely(t)
	t.Logf("Importing keys to gely")
	runGely(t, "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGelyWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gelyrpc {
	datadir := initGely(t)
	return startGelyWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
