...
func TestAccountListEmpty(t *testing.T) {
	gely := runGely(t, "account", "list")
	gely.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gely := runGely(t, "account", "list", "--datadir", datadir)
	defer gely.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gely := runGely(t, "account", "new", "--lightkdf")
	defer gely.ExpectExit()
	gely.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gely := runGely(t, "--lightkdf", "account", "import", keyfile, "-password", passwordFile)
	defer gely.ExpectExit()
	gely.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gely := runGely(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gely.ExpectExit()
	gely.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gely.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gely := runMinimalGely(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gely.StderrText(), m) {
...
// https://github.com/elysiumchain/go-elysium/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gely.SetTemplateFunc("keypath", func(file string) string {
