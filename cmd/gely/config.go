...
import (
	...
	"github.com/elysiumchain/go-elysium/accounts/external"
	"github.com/elysiumchain/go-elysium/accounts/keystore"
	"github.com/elysiumchain/go-elysium/accounts/scwallet"
	"github.com/elysiumchain/go-elysium/accounts/usbwallet"
...
func loadConfig(file string, cfg *gelyConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gely.ipc"
	return cfg
}

// makeConfigNode loads gely configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gelyConfig) {
	// Load defaults.
	cfg := gelyConfig{
...
// makeFullNode loads gely configuration and creates the Elysium backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil {
		...
		if err != nil {
			...
			log.Warn("Database has receipts with a legacy format. Please run `gely db freezer-migrate`.")
		}
	}
	...
	// Add the Elysium Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gelyConfig) {
