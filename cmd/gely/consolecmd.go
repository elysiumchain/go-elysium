...
import (
	...
	"github.com/elysiumchain/go-elysium/console"
...
var (
	...
	consoleCommand = cli.Command{
		...
		Description: `
The Gely console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gely.ethereum.org/docs/interface/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gely.ethereum.org/docs/interface/javascript-console.
This command allows to open a console on a running gely node.`,
	}
...
JavaScript API. See https://gely.ethereum.org/docs/interface/javascript-console`,
	}
)

// localConsole starts a new gely node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		return fmt.Errorf("Failed to attach to the inproc gely: %v", err)
	}
...
// remoteConsole will connect to a remote gely instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if endpoint == "" {
		...
		endpoint = fmt.Sprintf("%s/gely.ipc", path)
	}
	client, err := dialRPC(endpoint)
	if err != nil {
		utils.Fatalf("Unable to attach to remote gely: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.GlobalString(utils.JSpathFlag.Name),
...
// for "gely attach" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gely < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
...
// ephemeralConsole starts a new gely node, attaches an ephemeral JavaScript
