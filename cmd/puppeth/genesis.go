...
import (
	...
	math2 "github.com/elysiumchain/go-elysium/common/math"
...
// newAlethGenesisSpec converts a go-elysium genesis block into a Aleth-specific
// chain specification format.
func newAlethGenesisSpec(network string, genesis *core.Genesis) (*alethGenesisSpec, error) {
	// Only ethash is currently supported between go-elysium and aleth
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-elysium/pull/11039,
...
// newParityChainSpec converts a go-elysium genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-elysium and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// https://github.com/elysiumchain/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/elysiumchain/EIPs/blob/master/EIPS/eip-607.md
	...
	// gely has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Elysium.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.Elysium.MixHash = genesis.Mixhash[:]
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pyElysiumGenesisSpec represents the genesis specification format used by the
// Python Elysium implementation.
type pyElysiumGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyElysiumGenesisSpec converts a go-elysium genesis block into a Parity specific
// chain specification format.
func newPyElysiumGenesisSpec(network string, genesis *core.Genesis) (*pyElysiumGenesisSpec, error) {
	// Only ethash is currently supported between go-elysium and pyelysium
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyElysiumGenesisSpec{
