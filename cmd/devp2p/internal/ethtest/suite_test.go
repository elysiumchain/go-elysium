...
import (
	...
	"github.com/elysiumchain/go-elysium/internal/utesting"
...
func TestEthSuite(t *testing.T) {
	gely, err := runGely()
	if err != nil {
		t.Fatalf("could not run gely: %v", err)
	}
	defer gely.Close()

	suite, err := NewSuite(gely.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
...
// runGely creates and starts a gely node
func runGely() (*node.Node, error) {
...
func setupGely(stack *node.Node) error {
