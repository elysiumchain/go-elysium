...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Elysium address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Elysium account at a particular block.
    type Account {
    ...
    # Transaction is an Elysium transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Elysium account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Elysium account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Elysium block by number or by hash. If neither is
