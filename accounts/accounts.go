...
// Package accounts implements high level Elysium account management.
package accounts
...
// Account represents an Elysium account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Elysium account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain elysium.ChainStateReader)
...
//   keccak256("\x19Elysium Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Elysium Signed Message:\n%d%s", len(data), string(data))
