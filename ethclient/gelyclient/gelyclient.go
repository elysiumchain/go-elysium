...
// Package gelyclient provides an RPC client for gely-specific APIs.
package gelyclient
...
// Client is a wrapper around rpc.Client that implements gely-specific functionality.
//
// If you want to use the standardized Elysium RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg elysium.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg elysium.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gely node.
...
// MemStats retrieves the current memory stats from a gely node.
...
// GetNodeInfo retrieves the node info of a gely node.
