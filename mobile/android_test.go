...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gely.LightScryptN, Gely.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.elysium", new NodeConfig());
			node.start();
			...
			ElysiumClient ec = node.getElysiumClient();
	...
	// signatures too. Regression test for go-elysium issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gely-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/elysium/gelytest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gely and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.elysium", "github.com/elysiumchain/go-elysium/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gely.aar"), "gely.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "elysium", "gelytest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
