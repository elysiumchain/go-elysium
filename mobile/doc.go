...
// Package gely contains the simplified mobile APIs to go-elysium.
//
// The scope of this package is *not* to allow writing a custom Elysium client
// with pieces plucked from go-elysium, rather to allow writing native dapps on
...
// from the go-elysium repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-elysium version.
